...
class LDAModel(object):
    ...
    def __init__(self, dpre):
        ...
        # 修改：使用列表而不是numpy数组来存储Z，因为文档长度不同
        self.Z = []

        for x in range(dpre.docs_count):
            doc_z = []
...
def preprocessing():
    ...
    if dpre.docs_count == 0:
        logger.error(u"错误: 训练数据中没有有效的文档。请检查文件内容。")
        raise ValueError("No valid documents found in the training file.")
