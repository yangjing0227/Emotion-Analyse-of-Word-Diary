...
'''
警告的含义
这些警告都指向了你代码中的正则表达式字符串。在 Python 中，反斜杠 \ 是一个转义字符。当你在一个普通字符串中使用反斜杠时，
Python 解释器会尝试将其后面的字符解释为一个特殊的转义序列，例如 \n（换行符）或 \t（制表符）。

invalid escape sequence '\s': 在 re.sub('\s+', ' ', text_l) 中，\s 是一个正则表达式元字符，表示空白字符。
然而，在 Python 的普通字符串中，\s 没有特殊含义，所以 Python 解释器发出了警告。
invalid escape sequence '\[': 在 re.sub('#.+?#|\[.+?]|【.+?】', '', text_s) 中，\[ 本意是匹配字面上的中括号 [。
但是，在普通字符串中，\[ 不是一个已知的转义序列。
invalid escape sequence '\ ': 在 re.sub('@.+?[,，：:\ )]|@.+?$', '', text_s) 中，
\ （反斜杠后跟空格）也不是一个有效的转义序列。

如何改正
最简单且最标准的做法是使用 原始字符串（raw string）。
在 Python 中，原始字符串以 r 作为前缀，它会告诉解释器，字符串中的反斜杠字符不需要进行转义。
这样，正则表达式就可以按原样传递给 re.sub() 函数，避免了语法警告。
你需要将 re_sub 函数中的所有正则表达式字符串都修改为原始字符串。
'''
def re_sub(text_l):
    '替换文本中的超链接和多余的空格'
    if isinstance(text_l, str) and (text_l is not None):
        text_s = re.sub(r'\s+', ' ', text_l)
        text_s = re.sub(r' ', ',', text_s)
        text_s = re.sub(r'#.+?#|\[.+?]|【.+?】', '', text_s)
        text_s = re.sub(r'https?:[a-zA-Z/\\.0-9_]+', '', text_s)
        text_s = re.sub(r'@.+?[,，：: )]|@.+?$', '', text_s)
        text_s = re.sub(r'我在(\w){0,2}[:：](\w*)', '', text_s)
        text_s = re.sub(r'\[(\w){1,4}]', '', text_s)
        text_s = re.sub(r'&[a-z]+;', '', text_s)
    else:
        text_s = str(text_l)
...
class jieba4null():
    ...
    def __init__(self,n_core = 16):
        ...
        self.n_CORE=n_core
        # 修复Windows兼容性问题，因为并行模式仅支持POSIX系统
        # if os.name != 'nt':
...
def main(path_to_data, column_to_deal, output_file, input_ncores):
    ...
    writer = pd.ExcelWriter(output_file, engine='xlsxwriter')  # 推荐指定engine
...
if __name__ == '__main__':
    ...
    # 设置默认输入文件为 'my_data.xlsx'
    parser.add_argument('-i', '--inpath', dest='input_path', nargs='?', default='my_data.xlsx',
                        help='Name of the input excel file in folder:raw_data.')
    # 设置默认处理列为 '正文'
    parser.add_argument('-c', '--column', dest='column', nargs='?', default='正文',
                        help='Specify the column name of doc content. Default is "正文".')
